...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    """
    loss = foreground_img.new_tensor(0.0)
    ...
    mapped_bg_coords = fg_coords + torch.tensor(
        [shift_y, shift_x], device=fg_coords.device, dtype=fg_coords.dtype
    )

    bg_h, bg_w = blended_img.shape[-2:]
    valid = (
        (mapped_bg_coords[:, 0] >= 0) &
        (mapped_bg_coords[:, 0] < bg_h) &
        (mapped_bg_coords[:, 1] >= 0) &
        (mapped_bg_coords[:, 1] < bg_w)
    )
    if valid.sum().item() == 0:
        return loss

    fg_coords = fg_coords[valid]
    mapped_bg_coords = mapped_bg_coords[valid]

    bg_inside_mask = background_mask[0, 0, mapped_bg_coords[:, 0], mapped_bg_coords[:, 1]] > 0.5
    if bg_inside_mask.sum().item() == 0:
        return loss

    fg_coords = fg_coords[bg_inside_mask]
    mapped_bg_coords = mapped_bg_coords[bg_inside_mask]

    fg_values = foreground_laplacian[0, :, fg_coords[:, 0], fg_coords[:, 1]]
    bg_values = blended_laplacian[0, :, mapped_bg_coords[:, 0], mapped_bg_coords[:, 1]]
    loss = (bg_values - fg_values).pow(2).mean()
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    fg_coords = torch.nonzero(fg_mask_tensor[0, 0] > 0.5, as_tuple=False)
    bg_coords = torch.nonzero(bg_mask_tensor[0, 0] > 0.5, as_tuple=False)
    if fg_coords.numel() > 0 and bg_coords.numel() > 0:
        shift_y = int(bg_coords[:, 0].min().item() - fg_coords[:, 0].min().item())
        shift_x = int(bg_coords[:, 1].min().item() - fg_coords[:, 1].min().item())
        mapped_bg_coords = fg_coords + torch.tensor(
            [shift_y, shift_x], device=fg_coords.device, dtype=fg_coords.dtype
        )
        bg_h, bg_w = bg_img_tensor.shape[-2:]
        valid = (
            (mapped_bg_coords[:, 0] >= 0) &
            (mapped_bg_coords[:, 0] < bg_h) &
            (mapped_bg_coords[:, 1] >= 0) &
        ...
        if valid.sum().item() > 0:
            fg_coords = fg_coords[valid]
            mapped_bg_coords = mapped_bg_coords[valid]
            bg_inside_mask = bg_mask_tensor[0, 0, mapped_bg_coords[:, 0], mapped_bg_coords[:, 1]] > 0.5

            if bg_inside_mask.sum().item() > 0:
                fg_coords = fg_coords[bg_inside_mask]
                mapped_bg_coords = mapped_bg_coords[bg_inside_mask]
                blended_img[0, :, mapped_bg_coords[:, 0], mapped_bg_coords[:, 1]] = (
                    blended_img[0, :, mapped_bg_coords[:, 0], mapped_bg_coords[:, 1]] * 0.9 +
                    fg_img_tensor[0, :, fg_coords[:, 0], fg_coords[:, 1]] * 0.1
                )
...
def reset_polygon_selection(foreground_image_original, background_image_original):
    """
    Resets the polygon selection and related UI state.
    ...
    Returns:
        tuple: Reset foreground image, reset polygon state, reset background image,
        cleared blended output, and reset dx/dy values.
    """
    return (
...
""") as demo:
    ...
    reset_polygon_button.click(
        fn=reset_polygon_selection,
        inputs=[foreground_image_original, background_image_original],
        outputs=[foreground_image_with_polygon, polygon_state, background_image_with_polygon, output_image, dx, dy],
    )
